...
![ChatterCatcher hero](./assets/chattercatcher-hero.svg)
...
家庭里的重要信息常常散落在群聊里：课程时间、出行安排、付款信息、文件、截图和后续变更。ChatterCatcher 的目标是让这些信息自然沉淀为本地知识库，之后直接问群里的机器人即可。

```text
群里有人说：编程课的时间改成了后天13:40
你问机器人：@小陈 最近一次编程课是什么时候
机器人回答：后天13:40。来源：群成员在 2026/04/26 13:36 说“编程课的时间改成了后天13:40”。
```
...
硬性原则：事实性回答必须先检索证据，不能把大量历史聊天直接塞给 LLM。
...
| 配置项 | 说明 |
| --- | --- |
| 飞书 App ID | 飞书自建应用的 App ID |
| 飞书 App Secret | 写入 `secrets.json`，不会进入导出文件 |
| LLM Base URL | OpenAI-compatible chat completions endpoint |
| LLM API Key | 写入 `secrets.json` |
| LLM Model | 用于 RAG 答案生成 |
